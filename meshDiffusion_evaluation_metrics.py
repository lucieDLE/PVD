...
def read_surf(filepath, n_points=2048, scale_factor = None):
    ...
    points_array = np.zeros((num_points, 3), dtype=float)
    for i in range(num_points):
        points_array[i] = vtk_points.GetPoint(i)


    if num_points < n_points:
        num = n_points - num_points
        
        if num_points > 1000:
            neigh = NearestNeighbors(n_neighbors=points_array.shape[1]+1)
            neigh.fit(points_array)
            NearestNeighbors(algorithm='auto')
            indices = neigh.kneighbors(points_array, return_distance=False)
            list_point = []
            for i in range(int(num/2)+1):
                idx_1, idx_2, idx_3 = indices[i][0], indices[i][1], indices[i][2]

                new_point = (points_array[idx_1] + points_array[idx_2])/2
                list_point.append(new_point)

                new_point = (points_array[idx_1] + points_array[idx_3])/2
                list_point.append(new_point)

            new_pc = np.array(list_point)                        
            points_array = np.concatenate((points_array, new_pc[:num]))
            
    else:
        indices = np.random.choice(num_points, n_points, replace=False)
        points_array = points_array[indices, :]


    if scale_factor:
        points_array = np.multiply(points_array, scale_factor)
...
def create_batch(df):
    ...
    for idx, row in df.iterrows():
        f, s = row['surf'], row['surf_scale']

        points = read_surf(f,scale_factor=s)
        batch_array.append(points)

    return batch_array

def main(args):

    df_vtk_original= pd.read_csv(args.csv_original)
    df_vtk_samples= pd.read_csv(args.csv_sample)

    num_classes = np.unique(df_vtk_original['class'])

    for class_i in num_classes:
 
        df_original_i = df_vtk_original.loc[df_vtk_original['class']==class_i]
        n_samples = len(df_original_i)

        df_sample_i = df_vtk_samples.loc[df_vtk_samples['class']==class_i]
        df_sample_i = df_sample_i.sample(n_samples)

        sample_pcs = create_batch(df_sample_i)
        ref_pcs = create_batch(df_original_i)
        
        sample_pcs = sample_pcs.type(dtype=torch.FloatTensor)
        ref_pcs = ref_pcs.type(dtype=torch.FloatTensor)
        # Compute metrics
        results = compute_all_metrics(sample_pcs, ref_pcs, batch_size=1)
        results = {k: (v.cpu().detach().item() if not isinstance(v, float) else v) for k, v in results.items()}

        print(results)

        jsd = jsd_between_point_cloud_sets(sample_pcs.numpy(), ref_pcs.numpy())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_original', type=str, help='CSV file for testing', required=True)
    parser.add_argument('--csv_sample', type=str, help='CSV file for testing', required=True)


    parser.add_argument('--surf_column', help='surf column name', type=str, default='surf')
    parser.add_argument('--class_column', help='Class column', type=str, default='class')
    parser.add_argument('--scale', help='scale_factor', type=str, default='surf_scale')

    parser.add_argument('--out', help='Output directory', type=str, default="./")
    parser.add_argument('--out_csv', help='Output csv', type=str, default="./")
